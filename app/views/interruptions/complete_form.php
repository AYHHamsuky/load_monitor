<?php
/**
 * Stage 2: Complete 33kV Interruption
 * Path: /app/views/interruptions/complete_form.php
 */
...
.stage-badge { display:inline-flex; align-items:center; gap:6px; background:linear-gradient(135deg,#0b3a82,#1e40af); color:#fff; padding:6px 16px; border-radius:20px; font-size:12px; font-weight:700; margin-bottom:20px; }
.summary-box { background:linear-gradient(135deg,#eff6ff,#dbeafe); border:1px solid #93c5fd; border-radius:10px; padding:16px 20px; margin-bottom:20px; }
.summary-grid { display:grid; grid-template-columns:repeat(3,1fr); gap:12px; }
.summary-item .label { font-size:11px; font-weight:700; color:#1e40af; text-transform:uppercase; }
.summary-item .value { font-size:15px; font-weight:700; color:#0f172a; margin-top:2px; }
.serial-highlight { font-size:20px; color:#1e40af; font-family:monospace; font-weight:800; letter-spacing:1px; }
...
.form-group input:focus, .form-group select:focus, .form-group textarea:focus { outline:none; border-color:#1e40af; box-shadow:0 0 0 3px rgba(30,64,175,.1); }
.form-group textarea { resize:vertical; min-height:80px; font-family:inherit; }
#delayReasonGroup { display:none; }
.btn-group { display:flex; gap:12px; margin-top:24px; }
.btn-primary { background:linear-gradient(135deg,#0b3a82,#1e40af); color:#fff; border:none; padding:12px 28px; border-radius:8px; cursor:pointer; font-weight:600; font-size:14px; }
...
<div class="main-content">
  <div class="interruption-card">
    ...
    <h1 class="page-title">Complete 33kV Interruption</h1>
    ...
    <div class="summary-box">
      ...
      <div class="summary-grid">
        <div class="summary-item">
          <div class="label">33kV Feeder</div>
          <div class="value"><?= htmlspecialchars($interruption['fdr33kv_name']) ?></div>
        </div>
        <div class="summary-item">
    ...
    <form id="completeForm">
      ...
      <div class="btn-group">
        <a href="index.php?page=interruptions" class="btn-secondary">← Back to List</a>
...
document.getElementById('completeForm').addEventListener('submit', function(e) {
    ...
    fetch('ajax/interruption_complete.php', { method:'POST', body: new FormData(this) })
        .then(r => r.json())
        .then(res => {
