<?php
/**
 * Log New 33kV Interruption Form (Updated with Interruption Codes and Debugging)
...
function debugLog(message, data = null) {
    ...
    debugBox.innerHTML += logMessage + '\n';
    console.log(message, data);
}

debugLog('33kV Form initialized');
debugLog('Interruption codes loaded', Object.keys(interruptionCodes));
...
debugLog('Minimum datetime set', today);
...
document.getElementById('interruptionType').addEventListener('change', function() {
    ...
    debugLog('Interruption type changed', selectedType);
    ...
    if (selectedType && interruptionCodes[selectedType]) {
        debugLog('Populating codes for type', selectedType);
        interruptionCodes[selectedType].forEach(code => {
            const option = document.createElement('option');
            option.value = code.interruption_code;
            option.textContent = code.interruption_code; // CODE ONLY, NO DESCRIPTION
        ...
        debugLog('Codes populated', interruptionCodes[selectedType].length + ' codes');
    }
});
...
document.getElementById('interruptionCode').addEventListener('change', function() {
    ...
    if (selectedOption && selectedOption.value) {
        debugLog('Interruption code selected', selectedOption.value);
...
document.getElementById('interruptionForm').addEventListener('submit', function(e) {
    ...
    debugLog('Form data collected', formDataObj);
    
    // Validate required fields
    const requiredFields = ['fdr33kv_code', 'interruption_type', 'interruption_code', 
                           'datetime_out', 'datetime_in', 'load_loss', 'reason_for_interruption'];
    
    for (let field of requiredFields) {
        if (!formData.get(field)) {
            debugLog('Validation failed: missing field', field);
            alertBox.className = 'alert error';
            alertBox.textContent = '❌ Missing required field: ' + field;
    ...
    debugLog('Sending AJAX request to ajax/interruption_log.php');
    ...
    .then(text => {
        ...
        try {
            ...
            if (res.success) {
                ...
                debugLog('Success! Redirecting in 2 seconds...');
                ...
                debugLog('Server error', res.message);
            }
        } catch (parseError) {
            debugLog('JSON parse error', parseError.message);
            alertBox.className = 'alert error';
            alertBox.textContent = '❌ Invalid response from server. Check debug box below.';
            alertBox.style.display = 'block';
            submitBtn.disabled = false;
            submitBtn.textContent = 'Submit Interruption';
        }
