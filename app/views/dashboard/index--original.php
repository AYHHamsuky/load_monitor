    ...
    <div class="dashboard-container-11kv">
        ...
        <div class="page-header">
            <div>
                ...
                <p class="subtitle">ISS: <?= htmlspecialchars($iss['iss_name']) ?> | <?= date('l, F j, Y', strtotime($today)) ?> <small style="color:#f39c12;font-size:11px;">(operational day — closes 01:00)</small></p>
            </div>
...
function openLateEntryModal(serverData) {
    ...
    for (let h = serverData.hour_from; h <= serverData.hour_to; h++) {
        if (h >= currentSlot) continue; // skip current and future slots

        const opt = document.createElement('option');
        opt.value = h;
        opt.textContent = String(h).padStart(2, '0') + ':00';

        // Pre-select the hour that triggered this modal (from cell click)
        if (window._pendingCell && parseInt(window._pendingCell.hour) === h) {
            opt.selected   = true;
            hiddenHr.value = h;
        }
