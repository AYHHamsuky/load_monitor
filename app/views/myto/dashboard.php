 ...
 *
 * AJAX trend data is served by: public/ajax/myto_trend.php
 * (No AJAX handling in this view — that caused header-already-sent errors)
 */
?>
...
<div class="trend-card">
    ...
    <div class="trend-canvas-wrap" id="trendCanvasWrap">
        ...
        <div id="trendEmpty" style="display:none;text-align:center;padding:80px 20px;color:#cbd5e1;">
            ...
            <p style="font-size:14px;"><span id="trendEmptyMsg">Select a period above and click <strong style="color:#475569;">Refresh Chart</strong></span></p>
        </div>
...
<!-- ═══════════════════════════════════════════════════════════════════════════
     CHART.JS — single load, no date adapter needed
     ═══════════════════════════════════════════════════════════════════════════ -->
...
// ══════════════════════════════════════════════════════════════════════════════
//  MYTO TREND CHART
//  AJAX endpoint: myto_trend_ajax.php (standalone — no routing, no HTML output)
// ══════════════════════════════════════════════════════════════════════════════
...
// ── Data from PHP (emitted once) ──────────────────────────────────────────────
const ALL_TS_NAMES_MAP = <?= json_encode(array_column($all_ts, 'station_name', 'ts_code')) ?>;
...
function getSelectedTs() {
    const opts = Array.from(
        document.getElementById('trendTsSelect').selectedOptions
    ).map(o => o.value);
...
async function loadTrend() {
    ...
    // Build URL
    let url = `${TREND_AJAX_URL}?from=${encodeURIComponent(range.from)}&to=${encodeURIComponent(range.to)}`;
    ...
    try {
        ...
        try {
            ...
            console.error('Trend AJAX: JSON parse error. Response was:', text.substring(0, 400));
            elEmpty.style.display = 'block';
            document.getElementById('trendEmptyMsg').textContent =
                'Server error — check browser console (F12) for details.';
            return;
        }
        ...
        const hasData = data.days && data.days.some(d => d.actual !== null || d.myto !== null);
        if (!hasData) {
            elEmpty.style.display = 'block';
            document.getElementById('trendEmptyMsg').textContent =
                'No data found for the selected period and stations.';
            return;
        }

        trendLastData = { data, tsCodes };
        if (elChart) elChart.style.display = 'block';
        ...
        elEmpty.style.display = 'block';
        document.getElementById('trendEmptyMsg').textContent =
            'Network error — check browser console for details.';
    }
}

// ─── Re-render from cache when series checkbox changes ────────────────────────
function rerenderFromCache() {
    if (trendLastData) renderTrendChart(trendLastData.data, trendLastData.tsCodes);
}
...
function renderTrendChart(data, tsCodes) {
    ...
    if (isAll) {
        if (showActual) datasets.push({
            ...
            borderWidth: 2.5, pointRadius: nPts > 60 ? 0 : 3, pointHoverRadius: 5,
            fill: true, tension: .35, yAxisID: 'y', spanGaps: true,
        });
        if (showMyto) datasets.push({
            ...
            pointRadius: nPts > 60 ? 0 : 3, pointHoverRadius: 5,
            fill: true, tension: .35, yAxisID: 'y', spanGaps: true,
        });
        if (showVar) datasets.push({
            ...
            fill: false, tension: .35, yAxisID: 'yVar', spanGaps: true,
        });
    } else {
        tsCodes.forEach((tc, idx) => {
            const col  = tsColor(idx);
            const name = ALL_TS_NAMES_MAP[tc] || tc;
            ...
            if (showActual) datasets.push({
                ...
                pointRadius: nPts > 60 ? 0 : 2.5, pointHoverRadius: 4,
                fill: false, tension: .35, yAxisID: 'y', spanGaps: true,
            });
            if (showMyto) datasets.push({
                label: `${name} — MYTO`,
                data: data.days.map(d => tsd[d.date]?.myto ?? null),
                borderColor: col, borderDash: [5,3], borderWidth: 1.5,
                pointRadius: 0, fill: false, tension: .35, yAxisID: 'y', spanGaps: true,
            });
            if (showVar) datasets.push({
                label: `${name} — Variance`,
                data: data.days.map(d => tsd[d.date]?.variance ?? null),
                borderColor: col, borderDash: [2,4], borderWidth: 1.5,
                pointRadius: 0, fill: false, tension: .35, yAxisID: 'yVar', spanGaps: true,
            });
        });
    }
    ...
    const maxTicks = nPts > 90 ? 12 : (nPts > 30 ? 10 : nPts);
    const scales = {
        x: {
            type: 'category',
            grid: { color: 'rgba(0,0,0,.04)' },
            ticks: { color: '#64748b', font: { size: 11 }, maxTicksLimit: maxTicks, maxRotation: 30 },
        },
        y: {
            position: 'left',
...
function renderTrendKpi(data) {
    ...
    const set = (id, label, val, sub, color) => {
        ...
        if (!el) return;
        el.innerHTML = `<div class="tkpi-label">${label}</div>
    ...
    set('tkpiVar',    'Variance Total', sgn(s.total_variance)+fmt(s.total_variance)+' MW',
        'Actual − MYTO', col(s.total_variance));
    set('tkpiUtil',   'Avg Utilisation', pct(s.avg_util), 'Actual ÷ MYTO',
        s.avg_util !== null && s.avg_util > 100 ? '#d97706' : '#0891b2');
    set('tkpiDays',   'Days w/ Data',  `${s.days_with_data} / ${s.days}`, 'periods', '#6366f1');
}
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    if (fromEl) fromEl.value = fmtD(sub6);
    if (toEl)   toEl.value   = fmtD(now);
