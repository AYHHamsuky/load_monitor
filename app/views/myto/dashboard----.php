...
<div class="matrix-card">
    <div class="matrix-header">
        ...
        <span class="matrix-info">
            ...
            Each cell: <span class="legend-myto">MYTO alloc</span> /
                       <span class="legend-actual">Actual 33kV load</span>
            <?php if ($is_today): ?>
        ...
        <table class="metrics-table">
            <thead>
                <tr>
                    ...
                    <th>Total Actual 33kV Load (MW)</th>
                    <th>Variance (MW)</th>
                    <th>Fault Hours</th>
                    <th>Hours with MYTO</th>
                </tr>
            </thead>
            <tbody>
                <?php foreach ($matrix as $ts_code => $ts_data):
                ...
                <tr>
                    ...
                    <td class="metric-value"><?= number_format($ts_total_actual, 2) ?></td>
                    <td class="metric-variance <?= $ts_variance !== null
                        ? ($ts_variance > 0.5 ? 'var-over' : ($ts_variance < -0.5 ? 'var-under' : 'var-ok'))
                        : '' ?>">
                        <?= $ts_variance !== null
                            ? (($ts_variance > 0 ? '+' : '') . number_format($ts_variance, 2))
                            : '—' ?>
                    </td>
...
.cell-actual {display:block;color:#15803d;font-weight:700;font-size:10px;line-height:1.3;}
...
.modal-content{background:white;margin:5% auto;padding:0;border-radius:14px;
    width:90%;max-width:620px;box-shadow:0 12px 40px rgba(0,0,0,.3);
