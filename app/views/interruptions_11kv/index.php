...
<?php
// ── Status helpers ────────────────────────────────────────────────────────────
function statusBadge11(string $s): string {
    $map = [
...
function typeBadge11(string $t): string {
...
$feeder             = $feeder ?? ['fdr11kv_name'=>'All 11kV Feeders','parent_feeder'=>''];
?>
...
<div style="max-width:100%;padding:0;">
    ...
    <div style="background:white;padding:22px 28px;border-radius:12px;box-shadow:0 2px 8px rgba(0,0,0,.08);
        ...
        <div>
            <h1 style="font-size:24px;color:#2c3e50;margin-bottom:4px;">⚡ 11kV Interruption Records</h1>
            <p style="color:#7f8c8d;font-size:13px;">
                <?= htmlspecialchars($feeder['fdr11kv_name'] ?? 'All 11kV Feeders') ?>
                <?php if (!empty($feeder['parent_feeder'])): ?>
                    &nbsp;·&nbsp; <?= htmlspecialchars($feeder['parent_feeder']) ?>
                <?php endif; ?>
        ...
        <a href="index.php?page=interruptions_11kv&action=log"
    ...
    <div style="background:white;padding:18px 22px;border-radius:10px;box-shadow:0 2px 6px rgba(0,0,0,.06);margin-bottom:22px;">
        <form method="GET" style="display:flex;gap:12px;flex-wrap:wrap;align-items:flex-end;">
            <input type="hidden" name="page"   value="interruptions_11kv">
            <input type="hidden" name="action" value="list">
            <div style="flex:1;min-width:200px;">
                <label style="display:block;font-size:12px;font-weight:600;color:#555;margin-bottom:5px;">11kV Feeder</label>
                <select name="feeder" class="form-ctrl">
                    <option value="ALL" <?= $selectedFeederCode==='ALL'?'selected':'' ?>>All 11kV Feeders</option>
                    <?php foreach ($feeders_11kv as $f): ?>
                        <option value="<?= htmlspecialchars($f['fdr11kv_code']) ?>"
                                <?= $selectedFeederCode===$f['fdr11kv_code']?'selected':'' ?>>
                            <?= htmlspecialchars($f['fdr11kv_name']) ?>
                            <?php if (!empty($f['parent_feeder'])): ?>(<?= htmlspecialchars($f['parent_feeder']) ?>)<?php endif; ?>
                        </option>
                    <?php endforeach; ?>
                </select>
            </div>
            ...
            <a href="index.php?page=interruptions_11kv&action=list"
    ...
    <div style="background:white;border-radius:12px;box-shadow:0 2px 8px rgba(0,0,0,.08);overflow:hidden;">
        <div style="background:linear-gradient(135deg,#667eea,#764ba2);padding:16px 22px;
            ...
            <div style="display:flex;gap:10px;">
                ...
                <span style="color:white;border:1px solid rgba(255,255,255,.4);
        ...
        <table style="width:100%;border-collapse:collapse;font-size:13px;">
            <thead>
                <tr style="background:#f8f9fa;">
                    ...
                    <th class="th">11kV Feeder</th>
                    <th class="th">33kV Parent</th>
                    <th class="th">Type</th>
                    <th class="th">Code</th>
            ...
            <tr class="tr-row">
                ...
                <td class="td" style="font-weight:600;color:#2c3e50;"><?= htmlspecialchars($i['fdr11kv_name'] ?? '—') ?></td>
                <td class="td" style="color:#7f8c8d;font-size:12px;"><?= htmlspecialchars($i['parent_feeder'] ?? '—') ?></td>
                <td class="td"><?= typeBadge11($i['interruption_type'] ?? '') ?></td>
                ...
                <td class="td"><?= statusBadge11($i['form_status'] ?? '') ?></td>
                <td class="td" style="font-size:12px;color:#7f8c8d;"><?= htmlspecialchars($i['logger_name'] ?? '—') ?></td>
                <td class="td" style="white-space:nowrap;">
                    <a href="index.php?page=interruptions_11kv&action=view&ticket=<?= urlencode($i['ticket_number']) ?>"
                    ...
                    <a href="index.php?page=interruptions_11kv&action=complete&id=<?= (int)$i['id'] ?>"
                       title="Complete Stage 2" style="color:#27ae60;text-decoration:none;margin-right:8px;font-size:14px;">
                        <i class="fas fa-check-circle"></i>
                    </a>
                    <?php endif; ?>
                    <?php
                    // Edit/cancel allowed within 1-hour window (enforced server-side; link shown always, server rejects late)
                    if (in_array($i['form_status'],['PENDING_COMPLETION','AWAITING_APPROVAL'])):
                        $startedAt   = $i['started_at'] ?? $i['datetime_out'];
                        $withinHour  = $startedAt && (time() - strtotime($startedAt)) < 3600;
                    ?>
                    <?php if ($withinHour): ?>
                    <a href="index.php?page=interruptions_11kv&action=edit&ticket=<?= urlencode($i['ticket_number']) ?>"
                       title="Edit (within 1-hour window)" style="color:#f39c12;text-decoration:none;margin-right:8px;font-size:14px;">
                        <i class="fas fa-pen"></i>
                    </a>
                    <a href="index.php?page=interruptions_11kv&action=cancel&ticket=<?= urlencode($i['ticket_number']) ?>"
                       title="Cancel (within 1-hour window)"
                       style="color:#e74c3c;text-decoration:none;font-size:14px;"
                       onclick="return confirm('Cancel ticket <?= htmlspecialchars($i['ticket_number'],ENT_QUOTES) ?>? This cannot be undone.');">
