<?php
/**
 * Log New 11kV Interruption Form (Updated with Interruption Codes)
...
<div class="main-content">
    <div class="interruption-card">
        ...
        <div class="info-box">
            ⚠️ <strong>Important:</strong> Interruptions can only be logged for today or future dates. Past dates are not allowed. Duration will be auto-calculated.
        </div>
        ...
        <form id="interruptionForm">
                    ...
                    <select name="fdr11kv_code" id="feederSelect" required>
                        ...
                        <?php foreach ($feeders_11kv as $fdr): ?>
                            <option value="<?= htmlspecialchars($fdr['fdr11kv_code']) ?>" 
                                    <?= ($feeder['fdr11kv_code'] === $fdr['fdr11kv_code']) ? 'selected' : '' ?>>
                                <?= htmlspecialchars($fdr['fdr11kv_name']) ?>
                                <?php if (!empty($fdr['parent_feeder'])): ?>
                                    - <?= htmlspecialchars($fdr['parent_feeder']) ?>
                                <?php endif; ?>
                            </option>
                        <?php endforeach; ?>
                    </select>
            ...
            <div class="form-row">
                <div class="form-group">
                    ...
                    <input type="text" id="interruptionGroup" readonly placeholder="Auto-populated from code" style="background: #f9fafb;">
                </div>
                
                <div class="form-group">
                    <label>Body Responsible</label>
                    <input type="text" id="bodyResponsible" readonly placeholder="Auto-populated from code" style="background: #f9fafb;">
                </div>
            </div>
            ...
            <input type="hidden" name="interruption_group" id="interruptionGroupHidden">
            <input type="hidden" name="body_responsible" id="bodyResponsibleHidden">
...
// Set minimum date to today for both datetime fields
document.getElementById('datetimeOut').min = todayString;
document.getElementById('datetimeIn').min = todayString;
...
document.getElementById('interruptionType').addEventListener('change', function() {
    ...
    if (selectedType && interruptionCodes[selectedType]) {
        interruptionCodes[selectedType].forEach(code => {
            ...
            option.textContent = code.interruption_code + ' - ' + code.interruption_description;
...
document.getElementById('interruptionCode').addEventListener('change', function() {
    ...
    if (selectedOption && selectedOption.value) {
        ...
        document.getElementById('interruptionGroupHidden').value = selectedOption.dataset.group || '';
        document.getElementById('bodyResponsibleHidden').value = selectedOption.dataset.body || '';
        
        // Show approval notice if required
        ...
        document.getElementById('interruptionGroup').value = '';
        document.getElementById('bodyResponsible').value = '';
        document.getElementById('interruptionGroupHidden').value = '';
        document.getElementById('bodyResponsibleHidden').value = '';
        document.getElementById('approvalNotice').style.display = 'none';
        document.getElementById('requiresApproval').value = '0';
    }
});
...
// Validate datetime_out is not in the past
document.getElementById('datetimeOut').addEventListener('change', function() {
    const selectedDate = new Date(this.value);
    
    if (selectedDate < today) {
        alert('⚠️ Cannot log interruptions for past dates. Please select today or a future date.');
        this.value = '';
    }
});

// Validate datetime_in is after datetime_out and not in the past
document.getElementById('datetimeIn').addEventListener('change', function() {
    ...
    if (!dateOut) {
        alert('⚠️ Please select interruption time first');
        this.value = '';
        return;
    }
    ...
    if (selectedDateIn < today) {
        alert('⚠️ Restoration time cannot be in the past. Please select today or a future date.');
        this.value = '';
        return;
    }
...
document.getElementById('interruptionForm').addEventListener('submit', function(e) {
    ...
    const dateOut = new Date(document.getElementById('datetimeOut').value);
    const dateIn = new Date(document.getElementById('datetimeIn').value);
    
    if (dateOut < today || dateIn < today) {
        alertBox.className = 'alert error';
        alertBox.textContent = '❌ Cannot log interruptions with past dates';
    ...
    fetch('ajax/interruption_11kv_log.php', {
        method: 'POST',
        body: formData
    })
    .then(r => r.json())
    .then(res => {
        if (res.success) {
            ...
            setTimeout(() => {
                window.location.href = 'index.php?page=interruptions_11kv';
            }, 2000);
        } else {
            alertBox.className = 'alert error';
            alertBox.textContent = '❌ ' + res.message;
