...
// Populate codes when type is selected
document.getElementById('interruptionType').addEventListener('change', function () {
    ...
    debugLog('Interruption type changed', type);
    ...
    if (type && interruptionCodes[type]) {
        debugLog('Populating codes for type', type);
        interruptionCodes[type].forEach(code => {
            const opt = document.createElement('option');
            opt.value = code.interruption_code;
            opt.textContent = code.interruption_code; // CODE ONLY, NO DESCRIPTION
        ...
        debugLog('Codes populated', interruptionCodes[type].length + ' codes');
    }
});
...
document.getElementById('interruptionCode').addEventListener('change', function () {
    ...
    debugLog('Interruption code selected', opt.value);
...
document.getElementById('interruptionForm').addEventListener('submit', function(e) {
    ...
    const requiredFields = ['fdr11kv_code', 'interruption_type', 'interruption_code', 
    ...
    debugLog('Sending AJAX request to ajax/interruption_11kv_log.php');
