<?php
/**
 * Edit 11kV Interruption Form
 * Path: /app/views/interruptions_11kv/edit_form.php
 */
...
<div class="main-content">
    <div class="edit-card">
        <h1 class="page-title">✏️ Edit 11kV Interruption #<?= $interruption['id'] ?></h1>
        <p class="page-subtitle">Modify interruption details for <?= htmlspecialchars($interruption['fdr11kv_name']) ?></p>
        ...
        <form id="editInterruptionForm">
            ...
            <input type="hidden" name="fdr11kv_code" value="<?= $interruption['fdr11kv_code'] ?>">
            
            <div class="form-row">
                <div class="form-group">
                    <label>11kV Feeder</label>
                    <input type="text" value="<?= htmlspecialchars($interruption['fdr11kv_name']) ?>" disabled>
                </div>
            ...
            <div class="btn-group">
                <a href="index.php?page=interruptions_11kv&action=view&id=<?= $interruption['id'] ?>" class="btn-secondary">Cancel</a>
...
document.getElementById('editInterruptionForm')?.addEventListener('submit', function(e) {
    ...
    fetch('ajax/interruption_11kv_update.php', {
        method: 'POST',
    ...
    .then(res => {
        if (res.success) {
            ...
            setTimeout(() => {
                window.location.href = 'index.php?page=interruptions_11kv&action=view&id=<?= $interruption['id'] ?>';
            }, 1500);
        } else {
